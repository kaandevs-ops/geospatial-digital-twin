...
def _variation_params(position: Point2D, ftype: StreetFurnitureType) -> tuple[float, float]:
    key = f"{ftype.value}:{position.x:.4f}:{position.y:.4f}".encode("utf-8")
...
def _apply_variation(
    mesh: Mesh3D, position: Point2D, ground_z: float, scale: float, rotation_deg: float,
) -> Mesh3D:
    ...
    for v in mesh.vertices:
        ...
        new_vertices.append(
            Vertex3D(
                rx + position.x, ry + position.y, lz + ground_z,
                new_normal, v.tangent, v.uv,
            )
        )
...
class StreetFurnitureGenerator:
    ...
    @staticmethod
    def from_osm_tag(tag: str, position: Point2D, ground_z: float = 0.0) -> StreetFurnitureItem | None:
    ...
    def generate(item: StreetFurnitureItem, apply_variation: bool = True) -> Mesh3D:
        ...
        return _apply_variation(mesh, item.position, item.ground_z, scale, rotation_deg + item.rotation_deg)

    @staticmethod
    def street_lamp(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        pole = MeshBuilder.build_cylinder(
            radius=0.06, height=4.5, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=8, name="street_lamp_pole",
        )
        head = MeshBuilder.build_box(
            0.3, 0.3, 0.25, center_x=position.x, center_y=position.y,
            base_z=ground_z + 4.5, name="street_lamp_head",
        )
    ...
    def power_pole(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        pole = MeshBuilder.build_cylinder(
            radius=0.12, height=8.0, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=8, name="power_pole_shaft",
        )
        crossarm = MeshBuilder.build_box(
            1.8, 0.1, 0.1, center_x=position.x, center_y=position.y,
            base_z=ground_z + 7.5, name="power_pole_crossarm",
        )
    ...
    def trash_bin(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        return MeshBuilder.build_cylinder(
            radius=0.25, height=0.75, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=10, name="trash_bin",
        )

    @staticmethod
    def bench(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        seat = MeshBuilder.build_box(
            1.6, 0.45, 0.05, center_x=position.x, center_y=position.y,
            base_z=ground_z + 0.45, name="bench_seat",
        )
        leg_a = MeshBuilder.build_box(
            0.06, 0.45, 0.45, center_x=position.x - 0.7, center_y=position.y,
            base_z=ground_z, name="bench_leg_a",
        )
        leg_b = MeshBuilder.build_box(
            0.06, 0.45, 0.45, center_x=position.x + 0.7, center_y=position.y,
            base_z=ground_z, name="bench_leg_b",
        )
    ...
    def bus_stop(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        roof = MeshBuilder.build_box(
            3.0, 1.2, 0.08, center_x=position.x, center_y=position.y,
            base_z=ground_z + 2.2, name="bus_stop_roof",
        )
        back_wall = MeshBuilder.build_box(
            3.0, 0.05, 2.2, center_x=position.x, center_y=position.y - 0.55,
            base_z=ground_z, name="bus_stop_wall",
        )
        bench = StreetFurnitureGenerator.bench(
            Point2D(position.x, position.y - 0.3), ground_z=ground_z,
        )
    ...
    def drinking_water(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        column = MeshBuilder.build_cylinder(
            radius=0.1, height=0.9, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=8, name="drinking_water_column",
        )
        basin = MeshBuilder.build_cylinder(
            radius=0.22, height=0.08, center_x=position.x, center_y=position.y,
            base_z=ground_z + 0.85, segments=10, name="drinking_water_basin",
        )
    ...
    def fountain(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        basin = MeshBuilder.build_cylinder(
            radius=1.5, height=0.4, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=16, name="fountain_basin",
        )
        water = MeshBuilder.build_cylinder(
            radius=1.3, height=0.05, center_x=position.x, center_y=position.y,
            base_z=ground_z + 0.4, segments=16, name="fountain_water_surface",
        )
        centerpiece = MeshBuilder.build_cylinder(
            radius=0.15, height=1.2, center_x=position.x, center_y=position.y,
            base_z=ground_z + 0.4, segments=8, name="fountain_centerpiece",
        )
    ...
    def bicycle_rental(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        stand = MeshBuilder.build_box(
            2.4, 0.6, 0.9, center_x=position.x, center_y=position.y,
            base_z=ground_z, name="bicycle_rental_stand",
        )
        kiosk = MeshBuilder.build_box(
            0.6, 0.4, 1.3, center_x=position.x - 1.4, center_y=position.y,
            base_z=ground_z, name="bicycle_rental_kiosk",
        )
    ...
    def bicycle_parking(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        rail_a = MeshBuilder.build_box(
            1.6, 0.04, 0.6, center_x=position.x, center_y=position.y - 0.3,
            base_z=ground_z, name="bicycle_parking_rail_a",
        )
        rail_b = MeshBuilder.build_box(
            1.6, 0.04, 0.6, center_x=position.x, center_y=position.y + 0.3,
            base_z=ground_z, name="bicycle_parking_rail_b",
        )
    ...
    def taxi_stand(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        pole = MeshBuilder.build_cylinder(
            radius=0.05, height=2.2, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=8, name="taxi_stand_pole",
        )
        sign = MeshBuilder.build_box(
            0.4, 0.08, 0.3, center_x=position.x, center_y=position.y,
            base_z=ground_z + 2.0, name="taxi_stand_sign",
        )
    ...
    def traffic_signals(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        pole = MeshBuilder.build_cylinder(
            radius=0.06, height=2.8, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=8, name="traffic_signals_pole",
        )
        head = MeshBuilder.build_box(
            0.25, 0.25, 0.7, center_x=position.x, center_y=position.y,
            base_z=ground_z + 2.4, name="traffic_signals_head",
        )
    ...
    def pedestrian_crossing(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        ...
        return MeshBuilder.build_box(
            3.0, 2.0, 0.02, center_x=position.x, center_y=position.y,
            base_z=ground_z, name="pedestrian_crossing_patch",
        )

    @staticmethod
    def traffic_sign(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        pole = MeshBuilder.build_cylinder(
            radius=0.04, height=2.0, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=6, name="traffic_sign_pole",
        )
        plate = MeshBuilder.build_box(
            0.5, 0.05, 0.5, center_x=position.x, center_y=position.y,
            base_z=ground_z + 1.7, name="traffic_sign_plate",
        )
    ...
    def railway_station(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        ...
        building = MeshBuilder.build_box(
            8.0, 4.0, 3.5, center_x=position.x, center_y=position.y,
            base_z=ground_z, name="railway_station_building",
        )
        canopy = MeshBuilder.build_box(
            9.0, 5.0, 0.15, center_x=position.x, center_y=position.y,
            base_z=ground_z + 3.5, name="railway_station_canopy",
        )
        sign = MeshBuilder.build_box(
            1.2, 0.08, 0.5, center_x=position.x, center_y=position.y - 2.5,
            base_z=ground_z + 3.6, name="railway_station_sign",
        )
    ...
    def subway_entrance(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        canopy = MeshBuilder.build_box(
            2.5, 2.0, 0.15, center_x=position.x, center_y=position.y,
            base_z=ground_z + 2.2, name="subway_entrance_canopy",
        )
        posts = [
            MeshBuilder.build_cylinder(
                radius=0.08, height=2.2, center_x=position.x + dx, center_y=position.y + dy,
                base_z=ground_z, segments=6, name="subway_entrance_post",
            )
    ...
    def monument(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        base = MeshBuilder.build_box(
            1.4, 1.4, 0.5, center_x=position.x, center_y=position.y,
            base_z=ground_z, name="monument_base",
        )
        obelisk = MeshBuilder.build_box(
            0.5, 0.5, 3.5, center_x=position.x, center_y=position.y,
            base_z=ground_z + 0.5, name="monument_obelisk",
        )
    ...
    def artwork(position: Point2D, ground_z: float = 0.0) -> Mesh3D:
        base = MeshBuilder.build_cylinder(
            radius=0.6, height=0.4, center_x=position.x, center_y=position.y,
            base_z=ground_z, segments=12, name="artwork_base",
        )
        sculpture = MeshBuilder.build_cylinder(
            radius=0.25, height=2.0, center_x=position.x, center_y=position.y,
            base_z=ground_z + 0.4, segments=10, name="artwork_sculpture",
        )
