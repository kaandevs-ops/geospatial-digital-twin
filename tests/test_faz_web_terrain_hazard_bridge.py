...
def test_flat_terrain_has_zero_slope_and_low_landslide_risk():
    grid = DEMImporter.flat_terrain(width=10, height=10, resolution_m=2.0, elevation=5.0, origin=GeoPoint(0, 0))
...
def test_steep_ramp_scores_higher_landslide_risk_than_flat():
    ...
    ramp = HeightmapGrid(width=20, height=20, resolution_m=1.0, elevations=matrix, origin=GeoPoint(0, 0))
    flat = DEMImporter.flat_terrain(width=20, height=20, resolution_m=1.0, elevation=0.0, origin=GeoPoint(0, 0))
...
def test_valley_bottom_has_higher_flood_risk_than_ridge():
    ...
    width = 16
    matrix = [
        [abs(col - width // 2) * 2.0 for col in range(width)]
        for _row in range(width)
    ]
    grid = HeightmapGrid(width=width, height=width, resolution_m=2.0, elevations=matrix, origin=GeoPoint(0, 0))
    analyzer = TerrainHazardAnalyzer(grid)
...
def test_rainfall_missing_is_reported_as_no_data():
    grid = DEMImporter.flat_terrain(width=8, height=8, resolution_m=2.0, elevation=0.0, origin=GeoPoint(0, 0))
...
def test_top_risk_cells_sorted_descending():
    ...
    grid = HeightmapGrid(width=15, height=15, resolution_m=1.0, elevations=matrix, origin=GeoPoint(0, 0))
...
def test_session_terrain_hazard_end_to_end(tmp_path):
    ...
    session.terrain_brush(pid, "raise", center_x_m=20.0, center_y_m=20.0, radius_m=8.0, amount_m=12.0)
...
def test_rest_terrain_hazard_flow(tmp_path):
    ...
    router.dispatch("POST", f"/api/projects/{pid}/terrain/init", body={"width": 16, "height": 16, "resolution_m": 2.0})
    router.dispatch("POST", f"/api/projects/{pid}/terrain/brush",
                     body={"operation": "raise", "center_x_m": 16, "center_y_m": 16, "radius_m": 6, "amount_m": 10})

    r = router.dispatch("POST", f"/api/projects/{pid}/hazard/terrain-summary", body={})
    assert r.status == 200
    assert r.body["grid_width"] == 16

    r = router.dispatch("POST", f"/api/projects/{pid}/hazard/terrain-point", body={"x_m": 16, "y_m": 16})
    assert r.status == 200
    assert "landslide_index_0_100" in r.body

    r = router.dispatch("POST", f"/api/projects/{pid}/hazard/terrain-top-cells", body={"kind": "flood", "limit": 4})
    assert r.status == 200
