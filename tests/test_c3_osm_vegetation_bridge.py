...
class TestTreeInstanceFromPoint(unittest.TestCase):
    def test_point_feature_converts(self) -> None:
        feature = GeoFeature(
            ...
            properties={"natural": "tree", "leaf_type": "needleleaved",
                        "height": "8", "osm_id": 42},
        )
    ...
    def test_mesh_generation_does_not_raise(self) -> None:
        feature = GeoFeature(
            geometry_type="Point", coordinates=[0.0, 0.0], properties={},
        )
    ...
    def test_deterministic_same_seed_same_result(self) -> None:
        feature = GeoFeature(
            geometry_type="Point", coordinates=[1.0, 1.0],
            properties={"osm_id": 7},
        )
...
class TestForestScatter(unittest.TestCase):
    def _square_forest_feature(self, side_m: float = 40.0) -> GeoFeature:
        ...
        return GeoFeature(
            geometry_type="Polygon", coordinates=[ring],
    ...
    def test_scatter_produces_points_inside_polygon(self) -> None:
        ...
        result = scatter_forest_polygon(feature, density_per_100m2=2.0,
                                         min_distance_m=3.0, seed=1)
    ...
    def test_min_distance_is_respected(self) -> None:
        feature = self._square_forest_feature()
        result = scatter_forest_polygon(feature, density_per_100m2=5.0,
                                         min_distance_m=4.0, seed=2)
...
class TestGenerateVegetationForCollection(unittest.TestCase):
    def test_mixed_collection_only_processes_relevant_categories(self) -> None:
        tree = GeoFeature(
            geometry_type="Point", coordinates=[5.0, 5.0],
            properties={"__category__": "trees", "osm_id": 1},
        )
        forest_ring = [(100.0, 100.0), (130.0, 100.0), (130.0, 130.0), (100.0, 130.0), (100.0, 100.0)]
        forest = GeoFeature(
            geometry_type="Polygon", coordinates=[forest_ring],
            properties={"__category__": "forest", "osm_id": 2},
        )
        road = GeoFeature(
            geometry_type="LineString", coordinates=[(0.0, 0.0), (10.0, 10.0)],
