...
from harita.analysis_engine.visibility import (
    RayCasting, LineOfSight, ShadowAnalysis, BlindSpotAnalysis, VisibilityHeatmap,
)
from harita.analysis_engine.sun_simulation import (
    SolarPositionCalculator, SeasonalSunPath, SolarExposure, RoofIrradiance, ShadowProjection,
)
from harita.analysis_engine.environmental_sim import (
    WindSimulation, RainSimulation, FloodEstimation, HeatIslandSimulation,
    NoiseSimulation, ReflectionSimulation,
)
...
class TestShadowAnalysis:
    ...
    def test_daily_shadow_hours_bounded(self):
        ...
        hours = ShadowAnalysis.daily_shadow_hours((0, 0, 0.5), ISTANBUL, date, [mesh], step_minutes=60)
...
class TestVisibilityHeatmap:
    def test_full_visibility_no_occluders(self):
        cells = VisibilityHeatmap.compute(
            observers=[(0, 0, 5)], grid_origin=(0.0, 0.0),
            grid_width=3, grid_height=3, cell_size=5.0, z=0.0, occluders=[],
        )
        assert len(cells) == 9
...
class TestSeasonalSunPath:
    def test_sample_day_length(self):
        samples = SeasonalSunPath.sample_day(ISTANBUL, datetime(2026, 6, 21, tzinfo=timezone.utc), step_minutes=60)
...
class TestRoofIrradiance:
    def test_zero_at_night(self):
        sun = SolarPositionCalculator.compute(ISTANBUL, datetime(2026, 1, 1, 0, 0, tzinfo=timezone.utc))
...
class TestShadowProjection:
    def test_no_shadow_at_night(self):
        ...
        night_sun = SolarPositionCalculator.compute(ISTANBUL, datetime(2026, 1, 1, 0, 0, tzinfo=timezone.utc))
...
class TestWindSimulation:
    def test_obstacle_cell_has_zero_speed(self):
        ...
        field = WindSimulation.simulate(
            width=10, height=10, cell_size_m=1.0, obstacles=[obstacle],
    ...
    def test_wake_reduces_downstream_speed(self):
        ...
        field = WindSimulation.simulate(
            width=10, height=10, cell_size_m=1.0, obstacles=[obstacle],
            free_stream_speed=10.0, free_stream_direction_deg=0.0,
        )
...
class TestRainSimulation:
    def test_flow_direction_matrix_shape(self):
        grid = DEMImporter.synthetic_hills(width=8, height=8, resolution_m=1.0,
                                            origin=GeoPoint(0, 0), amplitude=5.0)
    ...
    def test_accumulation_non_negative(self):
        grid = DEMImporter.flat_terrain(width=5, height=5, resolution_m=1.0, elevation=0.0,
                                         origin=GeoPoint(0, 0))
...
class TestFloodEstimation:
    def test_flat_low_terrain_floods_fully(self):
        grid = DEMImporter.flat_terrain(width=6, height=6, resolution_m=1.0, elevation=0.0,
                                         origin=GeoPoint(0, 0))
    ...
    def test_high_terrain_not_flooded(self):
        grid = DEMImporter.flat_terrain(width=6, height=6, resolution_m=1.0, elevation=100.0,
                                         origin=GeoPoint(0, 0))
        result = FloodEstimation.estimate(grid, water_level_m=1.0)
        assert result.flooded_cell_count == 0

    def test_flooded_area_matches_cell_size(self):
        grid = DEMImporter.flat_terrain(width=4, height=4, resolution_m=2.0, elevation=0.0,
                                         origin=GeoPoint(0, 0))
        result = FloodEstimation.estimate(grid, water_level_m=1.0)
...
class TestReflectionSimulation:
    def test_reflect_point_across_ground_plane(self):
        reflected = ReflectionSimulation.reflect_point_across_plane(
            point=(0, 0, 5), plane_point=(0, 0, 0), plane_normal=(0, 0, 1),
        )
    ...
    def test_compute_path_longer_than_direct(self):
        result = ReflectionSimulation.compute(
            source=(0, 0, 2), receiver=(10, 0, 2),
            plane_point=(5, 5, 0), plane_normal=(0, 1, 0),
        )
