...
class TestTransitLine:
    ...
    def test_rejects_mismatched_travel_times(self):
        with pytest.raises(ValueError):
            TransitLine("L1", stops=["A", "B", "C"], travel_times_s=[100], schedule_headway_s=600)

    def test_cumulative_time(self):
        line = TransitLine(
            "L1", stops=["A", "B", "C"], travel_times_s=[100, 150], schedule_headway_s=600
        )
...
class TestTransitVehicle:
    def test_capacity_limited_boarding(self):
        line = TransitLine(
            "L1", stops=["A", "B"], travel_times_s=[100], schedule_headway_s=600, capacity=5
        )
...
class TestMultiModalRoute:
    def test_walk_transit_walk_chain(self):
        ...
        line = TransitLine("L1", stops=["A", "B"], travel_times_s=[300], schedule_headway_s=600)
        mm.add_line(line)
        ...
        assert result.total_duration_s == pytest.approx(sum(leg.duration_s for leg in result.legs))
    ...
    def test_transit_faster_than_pure_walk_for_long_distance(self):
        ...
        mm.add_stop(TransitStop("B", Point2D(10, 200), walk_node="stopB_walk"))
        mm.add_line(
            TransitLine("L1", stops=["A", "B"], travel_times_s=[300], schedule_headway_s=600)
        )
