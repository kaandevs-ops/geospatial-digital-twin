...
def test_apparent_temperature_matches_air_temp_when_no_radiant_gain_and_no_wind():
    apparent = ThermalComfort.apparent_temperature(
        air_temp_c=18.0, mean_radiant_temp_c=18.0, wind_speed_mps=0.0,
    )
...
def test_shadowed_calm_point_is_more_comfortable_than_sunny_windy_hot_point():
    ...
    shadow_result = ThermalComfort.evaluate(
        point=shadow_point, location=ISTANBUL, when_utc=JUNE_NOON,
        occluders=[occluder], air_temp_c=30.0, wind_speed_mps=0.5,
    )
    sunny_result = ThermalComfort.evaluate(
        point=sunny_point, location=ISTANBUL, when_utc=JUNE_NOON,
        occluders=[occluder], air_temp_c=30.0, wind_speed_mps=0.5,
    )
...
def test_night_time_has_no_irradiance_regardless_of_shadow():
    result = ThermalComfort.evaluate(
        point=(500.0, 500.0, 1.5), location=ISTANBUL, when_utc=MIDNIGHT,
        occluders=[], air_temp_c=15.0, wind_speed_mps=1.0,
    )
...
def test_category_labels_are_consistent_with_apparent_temperature_ordering():
    cold = ThermalComfort.evaluate(
        point=(500.0, 500.0, 1.5), location=ISTANBUL, when_utc=MIDNIGHT,
        occluders=[], air_temp_c=-10.0, wind_speed_mps=5.0,
    )
    comfortable = ThermalComfort.evaluate(
        point=(500.0, 500.0, 1.5), location=ISTANBUL, when_utc=MIDNIGHT,
        occluders=[], air_temp_c=20.0, wind_speed_mps=0.0,
    )
    hot = ThermalComfort.evaluate(
        point=(500.0, 500.0, 1.5), location=ISTANBUL, when_utc=JUNE_NOON,
        occluders=[], air_temp_c=40.0, wind_speed_mps=0.0,
    )
    assert cold.apparent_temperature_c < comfortable.apparent_temperature_c < hot.apparent_temperature_c
...
def test_evaluate_with_wind_field_samples_wake_reduced_speed_behind_building():
    ...
    poly = Polygon(points=[
        Point2D(45.0, 45.0), Point2D(55.0, 45.0), Point2D(55.0, 55.0), Point2D(45.0, 55.0),
    ])
    wind_field = WindSimulation.simulate(
        width=100, height=100, cell_size_m=1.0, obstacles=[poly],
        free_stream_speed=6.0, free_stream_direction_deg=0.0,
        heights_m=[20.0],
    )
    ...
    wake_result = ThermalComfort.evaluate_with_wind_field(
        point=wake_point, location=ISTANBUL, when_utc=MIDNIGHT, occluders=[],
        air_temp_c=25.0, wind_field=wind_field,
    )
    free_result = ThermalComfort.evaluate_with_wind_field(
        point=free_point, location=ISTANBUL, when_utc=MIDNIGHT, occluders=[],
        air_temp_c=25.0, wind_field=wind_field,
    )
...
def test_evaluate_with_wind_field_higher_wind_speed_lowers_apparent_temperature_with_radiant_gain():
    ...
    calm = ThermalComfort.evaluate_with_wind_field(
        point=calm_point, location=ISTANBUL, when_utc=JUNE_NOON, occluders=[],
        air_temp_c=30.0, wind_field=wind_field,
    )
    windy = ThermalComfort.evaluate_with_wind_field(
        point=windy_point, location=ISTANBUL, when_utc=JUNE_NOON, occluders=[],
        air_temp_c=30.0, wind_field=wind_field,
    )
