...
def _build_minimal_rinex304(tmp_path, num_epochs: int = 1):
    ver_line = (
        f"{3.04:9.2f}".ljust(20) + "O".ljust(20) + "G".ljust(20)
    )
    ...
    time_first_line = _line("  2024    1    1    0    0    0.0000000".ljust(60), "TIME OF FIRST OBS")
...
def test_missing_observation_field_is_not_filled_with_default(tmp_path):
    ...
    ver_line = _line(f"{3.04:9.2f}".ljust(20) + "O".ljust(20) + "G".ljust(20), "RINEX VERSION / TYPE")
    sys_obs_line = _line("G    3 C1C  L1C  S1C".ljust(60), "SYS / # / OBS TYPES")
    end_header_line = _line("", "END OF HEADER")
...
def test_unsupported_rinex2_version_rejected(tmp_path):
    ver_line = _line(f"{2.11:9.2f}".ljust(20) + "O".ljust(20) + "G".ljust(20), "RINEX VERSION / TYPE")
    end_header_line = _line("", "END OF HEADER")
